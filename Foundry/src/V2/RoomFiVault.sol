 ...
 * @notice Vault que genera yield depositando USDT en Acala (Polkadot)
 * @dev Usuarios (RentalAgreements) depositan USDT y ganan yield automáticamente
 *
 * ARQUITECTURA:
 * - Paseo testnet (Core) ↔ Acala (XCM)
 * - USDT como asset base (6 decimals)
 * - Yield farming en Acala Lending + DEX
 * - Split fees: 70% user, 30% protocol
 *
 * FLUJO DE FONDOS:
 * 1. RentalAgreement deposita security deposit → Vault
 * 2. Vault despliega 85% a AcalaYieldStrategy (15% buffer)
 * 3. Strategy genera yield 6-12% APY en Acala
...
contract RoomFiVault is Ownable, ReentrancyGuard {
     ...
     * @param _strategy Address de la strategy inicial (AcalaYieldStrategy)
    ...
    constructor(
        ...
        usdt = IERC20(_usdt);
        strategy = IAcalaYieldStrategy(_strategy);
        ...
        onlyAuthorized
    {
        ...
        // Deploy a strategy si hay cantidad significativa
        if (toDeploy > 0) {
    ...
    function updateStrategy(address newStrategy) external onlyOwner nonReentrant {
        ...
        // Set nueva strategy
        strategy = IAcalaYieldStrategy(newStrategy);
